...
original list.
-/
theorem length_dedup'_le {α : Type} [DecidableEq α] (l : List α) :
...
-/
theorem length_dedup'_cons_le {α : Type} [DecidableEq α] (a : α) (l : List α) :
...
theorem mem_dedup'_of_mem {α : Type} [DecidableEq α]
...
theorem mem_of_dedup' {α : Type} [DecidableEq α]
  ...
  exact fun h => mem_dedup'_of_mem l a h

theorem length_dedup'_cons_of_mem {α : Type} [DecidableEq α] (a : α) (l : List α)
  (h : a ∈ l) : (a :: l).dedup'.length = l.dedup'.length := by
  simp [dedup']
  have : a ∈ l.dedup' := mem_of_mem_dedup' l a h
  simp [this]

theorem length_dedup'_cons_of_not_mem {α : Type} [DecidableEq α] (a : α) (l : List α)
  ...
  · rename_i head tail ih
    ...
    obtain ⟨h1, h2⟩ := h
    split
    · have := @mem_dedup'_of_mem _ _ tail a
      simp_all
      omega
    · have := @mem_dedup'_of_mem _ _ tail a
      simp_all
...
theorem mem_append_left_of_mem_dedup' {α : Type} [DecidableEq α] (a : α) (l₁ l₂ : List α)
  ...
  a ∈ l₁ := by
  have := @mem_dedup'_of_mem _ _ (l₁ ++ l₂) a (by assumption)
  have := @mem_dedup'_of_mem _ _ l₂ a
  simp_all; cases this
  · assumption
...
theorem length_dedup'_append_le_sum {α : Type} [DecidableEq α] (l₁ l₂ : List α) :
  ...
  · rename_i head tail ih
    ...
    · have : head ∈ (tail ++ l₂).dedup' := by
        have := @mem_dedup'_of_mem _ _ tail head h1
    ...
    · simp_all
      ...
      · have : head ∈ (tail ++ l₂).dedup' := by
          have := @mem_dedup'_of_mem _ _ l₂ head  h2
          have := @mem_of_mem_dedup' _ _ (tail ++ l₂) head
          simp_all
        simp_all
        have := ih l₂
        omega
      · have : head ∉ (tail ++ l₂).dedup' := by
          have := @mem_dedup'_of_mem _ _ (tail ++ l₂) head
          intro h
          simp_all
...
theorem length_dedup'_of_removeAll {α : Type} [DecidableEq α] (a : α) (l : List α)
  (h : a ∈ l) :
  ...
  case cons x xs ih =>
    ...
    case neg =>
      ...
      · rename_i hx
        ...
        simp [h'] at this
        rw [this]
        have := @length_dedup'_cons_of_mem _ _ x (xs.removeAll [a])
      ...
      · rename_i h_x_not_in_xs
        ...
        simp [h'] at this
        rw [this]
        have := @length_dedup'_cons_of_not_mem _ _ x (xs.removeAll [a])
        have : ¬ x ∈ xs.removeAll [a] := by
          simp [removeAll]
          have : x ∉ xs := by
            have := @mem_of_dedup' _ _ xs x
...
theorem length_dedup'_append_le_left {α : Type} [DecidableEq α] (l₁ l₂ : List α) :
  ...
  case cons a as ih =>
    ...
    · by_cases ha : a ∈ (as ++ l₂).dedup'
      case pos =>
        ...
        have h := @length_dedup'_of_removeAll _ _ a (as ++ l₂) h_a_as_l2
        rw [h]
...
theorem length_dedup'_append_all_in_right {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h : l₁.all (fun e => e ∈ l₂)) :
  ...
  · rename_i head tail ih
    ...
    simp [@length_dedup'_cons_of_mem _ _ head (tail ++ l₂) h1']
...
theorem length_dedup'_append_subset_right {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h : l₁ ⊆ l₂) :
  (l₁ ++ l₂).dedup'.length = l₂.dedup'.length := by
  simp_all [List.instHasSubset, List.Subset]
  exact @length_dedup'_append_all_in_right _ _ l₁ l₂ (by simp_all)

theorem length_dedup'_append_all_in_left {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h : l₂.all (fun e => e ∈ l₁)) :
  (l₁ ++ l₂).dedup'.length = l₁.dedup'.length := by
  induction l₂ generalizing l₁
  case nil => simp_all
  case cons x xs ih =>
    ...
    simp_all
    obtain ⟨hx, h_x_l1⟩ := h
    have h_1 := @length_dedup'_of_removeAll _ _ x (l₁ ++ [x]) (by simp_all)
    have h_2 := @length_dedup'_of_removeAll _ _ x (l₁) (by simp_all)
    have h_3 : ((l₁ ++ [x]).removeAll [x]) = l₁.removeAll [x] := by
      simp [removeAll]
    simp_all

theorem length_dedup'_all_in_eq {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h1 : l₁.all (fun e => e ∈ l₂))
  (h2 : l₂.all (fun e => e ∈ l₁)) :
  l₁.dedup'.length = l₂.dedup'.length := by
  have h_1 := @length_dedup'_append_all_in_right _ _ l₁ l₂ h1
  have h_2 := @length_dedup'_append_all_in_left _ _ l₁ l₂ h2
  simp_all

theorem length_dedup'_subset_eq {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h1 : l₁ ⊆ l₂) (h2 : l₂ ⊆ l₁) :
  l₁.dedup'.length = l₂.dedup'.length := by
  have := @length_dedup'_all_in_eq _ _ l₁ l₂
  simp_all [List.instHasSubset, List.Subset]

theorem length_dedup'_append_le_right {α : Type} [DecidableEq α] (l₁ l₂ : List α) :
  l₂.dedup'.length ≤ (l₁ ++ l₂).dedup'.length := by
  have h_left := @length_dedup'_append_le_left _ _ l₂ l₁
  have := @length_dedup'_all_in_eq _ _ (l₁ ++ l₂) (l₂ ++ l₁)
  simp_all

theorem length_dedup'_of_all_in_not_mem_lt {α : Type} [DecidableEq α] (l₁ l₂ : List α) (a : α)
  ...
  | cons head tail ih =>
    ...
    · rename_i h_head_not_in_tail
      have h_head_tail := @length_dedup'_cons_of_not_mem _ _ head tail
      by_cases h_head_in_tail : head ∈ tail
      case pos =>
        simp_all [@mem_of_dedup' _ _ tail head]
      case neg =>
        have h_removeAll := @length_dedup'_of_removeAll _ _ head l₂ h1_head_l2
        simp_all
...
theorem length_dedup'_of_subset_not_mem_lt {α : Type} [DecidableEq α] (l₁ l₂ : List α) (a : α)
  (h1 : l₁ ⊆ l₂) (h2 : a ∉ l₁) (h3 : a ∈ l₂) :
  l₁.dedup'.length < l₂.dedup'.length := by
  have := @length_dedup'_of_all_in_not_mem_lt _ _ l₁ l₂ a
  simp_all [List.instHasSubset, List.Subset]

theorem length_dedup'_of_subset_le {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  ...
  | cons head tail ih =>
    ...
    case pos =>
      have := @length_dedup'_cons_of_mem _ _ head tail h_head
      exact le_of_eq_of_le this (ih h_tail_l2)
    case neg =>
      simp_all
      have := @length_dedup'_of_subset_not_mem_lt _ _ tail l₂ head h_tail_l2 h_head h
