...
def substituteIFIS (isctx : ISContext) (iF : Boogie.SMT.IF) : Boogie.SMT.IF :=
...
noncomputable def denoteBoolTermFromContext
    (uss : USContext) (iss : ISContext) (ufs : UFContext) (ifs : List Boogie.SMT.IF) (t : Term) := do
  ...
  let ⟨.prim .bool, _, ft⟩ ← denoteTerm ⟨⟨uss, iss⟩, ⟨{}, ifs.map Boogie.SMT.IF.uf ++ ufs⟩⟩ t | none
...
where
  ...
  @[simp]
  bindIFs sctx vs ufs (ifs : List _) (ft' : TermDenoteInput ⟨sctx, ⟨vs, ifs.map Boogie.SMT.IF.uf ++ ufs⟩⟩ → Prop) :
      Option (TermDenoteInput ⟨sctx, ⟨vs, ufs⟩⟩ → Prop) :=
    do match ifs with
    | [] => return ft'
    | { uf := _, body } :: ifs =>
      let ft ← bindIF body { sctx, tctx := { vs := vs, ufs := ifs.map Boogie.SMT.IF.uf ++ ufs } } ft'
