...
abbrev USContext := List Boogie.SMT.Sort
    ...
    if hTy : (denoteSort ctx.sctx ty).isSome then
      ...
      let ft (tdi : TermDenoteInput ctx) :=
        ∀ x : (denoteSort ctx.sctx ty).get hTy ⟨tdi.sΓ, tdi.hsΓ⟩,
          let var' := { var := { isBound := true, id := n, ty := ty }, h := hTy, varΓ := x }
...
@[simp]
noncomputable def bindUS {us'} (sctx : SortContext) (ft' : SortDenoteInput (us' :: sctx) → Prop) :
  Option (SortDenoteInput sctx → Prop) := do
  let sctx' := us' :: sctx
  let ft (tdi : SortDenoteInput sctx) :=
    set_option checkBinderAnnotations false in
    ∀ (α : mkTypeFunType us'.arity) [inst : mkNonemptyPred α],
      let us' := { us := us', usΓ := α, nonempty := inst }
      let usΓ' : USEnvironment := us' :: tdi.sΓ
      haveI hus' : usΓ'.WF sctx' :=
        ...
        have ha' := fun i hius => match i with
          ...
          | i + 1 =>
            ...
            (List.getElem_cons_succ _ sctx i hius).symm ▸ (List.getElem_cons_succ _ tdi.sΓ i hiusΓ).symm ▸ tdi.hsΓ.ha i hius'
        { h := h', ha := ha' }
...
/--
Extend the context with a free variable and push its value into the denotation.
-/
@[simp]
noncomputable def bindFV {n} {ty} (ctx : Context) (ft' :
    let v' := { isBound := false, id := n, ty := ty }
    let vs' := v' :: ctx.tctx.vs
    let tctx' := { ufs := ctx.tctx.ufs, vs := vs' }
...
-/
@[simp]
noncomputable def denoteBoolTermFromContext (uss vs ufs) (t : Term) := do
  let ⟨.prim .bool, _, ft⟩ ← denoteTerm ⟨uss, ⟨vs, ufs⟩⟩ t | none
  let ft ← bindUFs uss vs ufs ft
  let ft ← bindFVs uss vs [] ft
  let ft ← bindUSs uss fun ⟨sΓ, hsΓ⟩ =>
...
where
  @[simp]
  bindUSs uss (ft' : SortDenoteInput uss → Prop) : Option (SortDenoteInput {} → Prop) :=
    do match uss with
    | [] => return ft'
    | _ :: uss =>
      let ft ← bindUS uss ft'
      bindUSs uss ft
  @[simp]
  bindFVs sctx vs ufs (ft' : TermDenoteInput ⟨sctx, ⟨vs, ufs⟩⟩ → Prop) : Option (TermDenoteInput ⟨sctx, ⟨{}, ufs⟩⟩ → Prop) :=
    do match vs with
    | [] => return ft'
    | { isBound := false, id := _, ty := _ } :: vs =>
      let ft ← bindFV { sctx, tctx := { vs, ufs } } ft'
      bindFVs sctx vs ufs ft
    | _ => none
  @[simp]
...
noncomputable def denoteQuery (ctx : Boogie.SMT.Context) (assums : List Term) (conc : Term) : Option Prop :=
  ...
  match denoteBoolTermFromContext uss vs ufs t with
  | none => none
  | some ft =>
...
example :
  let a := .var { isBound := true, id := "a", ty := .prim .int }
  (denoteBoolTermAux (.quant .all [("a", .prim .int)] a (.app .gt [.prim (.int 42), a] (.prim .int)))) =
  .some (∀ (x : Int), 42 > x) := by
  rfl

example :
  let a := .var { isBound := true, id := "a", ty := .prim (.bitvec 32) }
  (denoteQuery {} [] (.quant .all [("a", .prim (.bitvec 32))] a (.app .bvugt [.prim (.bitvec (42 : BitVec 32)), a] (.prim (.bitvec 32))))) =
  .some (∀ (x : BitVec 32), 42 > x) := by
  rfl

example :
  let α := { name := "α", arity := 0 }
  let a := { isBound := false, id := "a", ty := .constr α.name [] }
  (denoteQuery { sorts := #[α], fvs := #[a] } [] (.app .eq [a, a] (.prim .bool))) =
  .some (∀ (α : Type) [Nonempty α] (x : α), x = x) := by
  rfl

example :
  let α := { name := "α", arity := 1 }
  let a := { isBound := false, id := "a", ty := .constr α.name [.prim .int] }
  (denoteQuery { sorts := #[α], fvs := #[a] } [] (.app .eq [a, a] (.prim .bool))) =
...
example :
  ...
  (denoteQuery { sorts := #[α, β], fvs := #[a] } [] (.app .eq [a, a] (.prim .bool))) =
  .some (∀ (α : Type → Type → Type) [∀ x y, Nonempty (α x y)] (β : Type) [Nonempty β] (x : α β Prop), x = x) := by
  rfl

example :
  let a := { isBound := false, id := "a", ty := .prim .int }
  (denoteQuery { fvs := #[a] } [] (.app .gt [.prim (.int 42), a] (.prim .int))) =
  .some (∀ (x : Int), 42 > x) := by
  rfl
...
example :
  let f := { id := "f", args := [{ isBound := true, id := "a", ty := .prim .int }], out := .prim .int }
...
example :
  let ctx := {
      sorts := #[],
      fvs := #[{ isBound := false, id := "$__n0", ty := TermType.prim (TermPrimType.int) }],
      ufs := #[],
      ifs := #[],
      axms := #[],
      tySubst := [] }
  let ts := [Term.app
       ...
       [Term.prim (TermPrim.int 0),
        Term.var
          { isBound := false, id := "$__n0", ty := TermType.prim (TermPrimType.int) }]
