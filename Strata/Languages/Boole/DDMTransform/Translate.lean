...
import Strata.Languages.Boole.BooleGen
import Strata.Languages.Boole.Factory
...
open Strata Boole Lambda Imperative Lean.Parser
...
instance : ToString (Boole.Program × Array String) where
...
structure TransBindings where
  ...
  boundVars : Array (LExpr BooleLParams.mono) := #[]
  freeVars  : Array Boole.Decl := #[]
...
instance : Inhabited (List Boole.Statement × TransBindings) where
  default := ([], {})

instance : Inhabited Boole.Decl where
  default := .var "badguy" (.forAll [] (.tcons "bool" [])) (.false ())

instance : Inhabited (Procedure.CheckAttr) where
  default := .Default

instance : Inhabited (Boole.Decl × TransBindings) where
  default := (.var "badguy" (.forAll [] (.tcons "bool" [])) (.false ()), {})

instance : Inhabited (Boole.Decls × TransBindings) where
  default := ([], {})

instance : Inhabited (List BooleIdent × TransBindings) where
  default := ([], {})
  ...
  | .fvar _ i argst =>
    ...
    let ty_core ← match decl with
                  | .type (.con tcons) =>
                    ...
                    -- Boole is concerned. See `Boole.TypeConstructor`, where there is no
...
def translateTypeSynonym (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_typesynonym 4
  ...
  let decl := Boole.Decl.type (.syn { name := name, typeArgs := targs, type := typedef })
  return (decl, { bindings with freeVars := bindings.freeVars.push decl })


def translateTypeDecl (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_typedecl 2
  let name ← translateIdent TyIdentifier op.args[0]!
  let numargs ←
    translateOption
      (fun maybearg =>
  ...
  -- irrelevant.
  let decl := Boole.Decl.type (.con { name := name, numargs := numargs })
  return (decl, { bindings with freeVars := bindings.freeVars.push decl })

---------------------------------------------------------------------

def translateLhs (arg : Arg) : TransM BooleIdent := do
  let .op op := arg
    | TransM.error s!"translateLhs expected op {repr arg}"
  match op.name, op.args with
  | q`Boogie.lhsIdent, #[id] => translateIdent BooleIdent id
...
def translateBindMk (bindings : TransBindings) (arg : Arg) :
   TransM (BooleIdent × List TyIdentifier × LMonoTy) := do
  let .op op := arg
    | TransM.error s!"translateBindMk expected op {repr arg}"
  match op.name, op.args with
  | q`Boogie.bind_mk, #[ida, targsa, tpa] =>
    let id ← translateIdent BooleIdent ida
...
partial def dealiasTypeExpr (p : Strata.Program) (te : TypeExpr) : TypeExpr :=
  match te with
  | (.fvar _ idx #[]) =>
...
def dealiasTypeArg (p : Strata.Program) (a : Arg) : Arg :=
  match a with
...
def translateFn (ty? : Option LMonoTy) (q : QualifiedIdent) : TransM Boole.Expression.Expr :=
  match ty?, q with
...
def translateQuantifier
  ...
  TransM Boole.Expression.Expr := do
...
partial
def translateTriggerGroup (p: Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM Boole.Expression.Expr := do
  let .op op := arg
...
partial
def translateTriggers (p: Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM Boole.Expression.Expr := do
  let .op op := arg
...
partial def translateExpr (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM Boole.Expression.Expr := do
  let .expr expr := arg
  ...
  | .fn _ q`Boogie.map_set, [_ktp, _vtp, ma, ia, xa] =>
     let kty ← translateLMonoTy bindings _ktp
     let vty ← translateLMonoTy bindings _vtp
     -- TODO: use Boole.mapUpdateOp, but specialized
     let fn : LExpr BooleLParams.mono := (LExpr.op () "update" (.some (LMonoTy.mkArrow (mapTy kty vty) [kty, vty, mapTy kty vty])))
     let m ← translateExpr p bindings ma
     let i ← translateExpr p bindings ia
     let x ← translateExpr p bindings xa
  ...
  | .fvar _ i, argsa =>
    -- Call of a function declared/defined in Boole.
    assert! i < bindings.freeVars.size
    let decl := bindings.freeVars[i]!
    match decl with
    | .func func =>
...
partial def translateExprs (p : Strata.Program) (bindings : TransBindings) (args : Array Arg) :
  TransM (Array Boole.Expression.Expr) :=
  args.mapM (fun a => translateExpr p bindings a)
end

---------------------------------------------------------------------

def translateInvariant (p : Strata.Program) (bindings : TransBindings) (arg : Arg) : TransM (Option Expression.Expr) := do
  match arg with
  | .option _ (.some m) => do
...
def initVarStmts (tpids : ListMap Expression.Ident LTy) (bindings : TransBindings) :
  ...
  | [] => return ([], bindings)
  | (id, tp) :: rest =>
    let s := Boole.Statement.init id tp (Names.initVarValue (id.name ++ "_" ++ (toString bindings.gen.var_def)))
  ...
  if decls.size != 1 then
    ...
    let newVars ← tpids.mapM (fun (id, ty) =>
                    if h: ty.isMonoType then
                      return ((LExpr.fvar () id (ty.toMonoType h)): LExpr BooleLParams.mono)
                    else
...
def translateInitStatement (p : Strata.Program) (bindings : TransBindings) (args : Array Arg) :
  TransM ((List Boole.Statement) × TransBindings) := do
  if args.size != 3 then
    TransM.error "translateInitStatement unexpected arg length {repr decls}"
  else
    let mty ← translateLMonoTy bindings args[0]!
    let lhs ← translateIdent BooleIdent args[1]!
    let val ← translateExpr p bindings args[2]!
    let ty := (.forAll [] mty)
    let newBinding: LExpr BooleLParams.mono := LExpr.fvar () lhs mty
    let bbindings := bindings.boundVars ++ [newBinding]
    return ([.init lhs ty val], { bindings with boundVars := bbindings })

mutual
partial def translateStmt (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM (List Boole.Statement × TransBindings) := do
  let .op op := arg
  ...
  | q`Boole.for_statement, #[va, ina, ga, sa, ia, ba] =>
    ...
    let g ← translateExpr p bindings ga
    let step ← translateExpr p bindings sa
    let i ← translateInvariant p bindings ia
    let (bodyss, bindings) ← translateBlock p bindings ba
...
partial def translateBlock (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM ((List Boole.Statement) × TransBindings) := do
...
partial def translateElse (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
  TransM ((List Boole.Statement) × TransBindings) := do
  let .op op := arg
...
def translateModifies (arg : Arg) : TransM BooleIdent := do
  let args ← checkOpArg arg q`Boogie.modifies_spec 1
  translateIdent BooleIdent args[0]!
...
def translateRequires (p : Strata.Program) (name : BooleIdent) (count : Nat) (bindings : TransBindings) (arg : Arg) :
  TransM (ListMap BooleLabel Procedure.Check) := do
...
def translateEnsures (p : Strata.Program) (name : BooleIdent) (count : Nat) (bindings : TransBindings) (arg : Arg) :
  TransM (ListMap BooleLabel Procedure.Check) := do
...
def translateSpecElem (p : Strata.Program) (name : BooleIdent) (count : Nat) (bindings : TransBindings) (arg : Arg) :
  TransM (List BooleIdent × ListMap BooleLabel Procedure.Check × ListMap BooleLabel Procedure.Check) := do
  let .op op := arg
...
partial def translateSpec (p : Strata.Program) (name : BooleIdent) (bindings : TransBindings) (arg : Arg) :
  TransM (List BooleIdent × ListMap BooleLabel Procedure.Check × ListMap BooleLabel Procedure.Check) := do
...
def translateProcedure (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_procedure 6
  let pname ← translateIdent BooleIdent op.args[0]!
...
def translateConstant (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_constdecl 3
  let cname ← translateIdent BooleIdent op.args[0]!
  let typeArgs ← translateTypeArgs op.args[1]!
...
def translateAxiom (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_axiom 2
...
def translateDistinct (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_distinct 2
...
def translateFunction (status : FnInterp) (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ←
    match status with
    | .Definition  => @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_fndef  6
    | .Declaration => @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_fndecl 4
  let fname ← translateIdent BooleIdent op.args[0]!
  let typeArgs ← translateTypeArgs op.args[1]!
  let sig ← translateBindings bindings op.args[2]!
...
def translateGlobalVar (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Boogie.command_var 1
...
partial def translateBooleDecls (p : Strata.Program) (bindings : TransBindings) :
  TransM Boole.Decls := do
  let (decls, _) ← go 0 p.commands.size bindings p.commands
  return decls
  where go (count max : Nat) bindings ops : TransM (Boole.Decls × TransBindings) := do
  match (max - count) with
  | 0 => return ([], bindings)
  | _ + 1 =>
    let op := ops[count]!
    let (decl, bindings) ←
      ...
      | _ => TransM.error s!"translateBooleDecls unimplemented for {repr op}"
    let (decls, bindings) ← go (count + 1) max bindings ops
    return ((decl :: decls), bindings)

def translateProgram (p : Strata.Program) : TransM Boole.Program := do
  let decls ← translateBooleDecls p {}
  return { decls := decls }
