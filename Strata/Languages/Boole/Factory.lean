...
-/
def ToBooleIdent {M: Type} (ine: LExpr (@TImplicit M Unit)): LExpr (@TImplicit M Visibility) :=
match ine with
    | .const m c => .const m c
    | .op m o oty => .op m (BooleIdent.unres o.name) oty
    | .bvar m deBruijnIndex => .bvar m deBruijnIndex
    | .fvar m name oty => .fvar m (BooleIdent.unres name.name) oty
    | .abs m oty e => .abs m oty (ToBooleIdent e)
    | .quant m k oty tr e => .quant m k oty (ToBooleIdent tr) (ToBooleIdent e)
    | .app m fn e => .app m (ToBooleIdent fn) (ToBooleIdent e)
    | .ite m c t e => .ite m (ToBooleIdent c) (ToBooleIdent t) (ToBooleIdent e)
    | .eq m e1 e2 => .eq m (ToBooleIdent e1) (ToBooleIdent e2)
...
variable [Coe String BooleLParams.Identifier]
...
elab "ExpandBVOpFuncDefs" "[" sizes:num,* "]" : command => do
  for size in sizes.getElems do
    ...
    for (op, arity) in List.zip BVOpNames BVOpAritys do
      ...
      elabCommand (← `(def $funcName : LFunc BooleLParams := $funcArity $opName mty[$(mkIdent bvTypeName):ident] none))

ExpandBVOpFuncDefs[1, 2, 8, 16, 32, 64]

/- Real Arithmetic Operations -/

def realAddFunc : LFunc BooleLParams := binaryOp "Real.Add" mty[real] none
def realSubFunc : LFunc BooleLParams := binaryOp "Real.Sub" mty[real] none
def realMulFunc : LFunc BooleLParams := binaryOp "Real.Mul" mty[real] none
def realDivFunc : LFunc BooleLParams := binaryOp "Real.Div" mty[real] none
def realNegFunc : LFunc BooleLParams := unaryOp "Real.Neg" mty[real] none

/- Real Comparison Operations -/
def realLtFunc : LFunc BooleLParams := binaryPredicate "Real.Lt" mty[real] none
def realLeFunc : LFunc BooleLParams := binaryPredicate "Real.Le" mty[real] none
def realGtFunc : LFunc BooleLParams := binaryPredicate "Real.Gt" mty[real] none
def realGeFunc : LFunc BooleLParams := binaryPredicate "Real.Ge" mty[real] none

/- String Operations -/
def strLengthFunc : LFunc BooleLParams :=
    { name := "Str.Length",
      typeArgs := [],
      inputs := [("x", mty[string])]
      output := mty[int],
      concreteEval := some (unOpCeval (T:=BooleLParams) String Int (.intConst (T:=BooleLParams.mono)) (@LExpr.denoteString BooleLParams)
                            (fun s => (Int.ofNat (String.length s))))}

def strConcatFunc : LFunc BooleLParams :=
    { name := "Str.Concat",
      typeArgs := [],
      inputs := [("x", mty[string]), ("y", mty[string])]
      output := mty[string],
      concreteEval := some (binOpCeval String String (.strConst (T := BooleLParams.mono))
                            LExpr.denoteString String.append)}

def strSubstrFunc : LFunc BooleLParams :=
    { name := "Str.Substr",
      typeArgs := [],
...
def strToRegexFunc : LFunc BooleLParams :=
    { name := "Str.ToRegEx",
      typeArgs := [],
      inputs := [("x", mty[string])]
      output := mty[regex] }

def strInRegexFunc : LFunc BooleLParams :=
    { name := "Str.InRegEx",
      typeArgs := [],
      inputs := [("x", mty[string]), ("y", mty[regex])]
      output := mty[bool] }

def reAllCharFunc : LFunc BooleLParams :=
    { name := "Re.AllChar",
      typeArgs := [],
      inputs := []
      output := mty[regex] }

def reAllFunc : LFunc BooleLParams :=
    { name := "Re.All",
      typeArgs := [],
      inputs := []
      output := mty[regex] }

def reRangeFunc : LFunc BooleLParams :=
    { name := "Re.Range",
      typeArgs := [],
      inputs := [("x", mty[string]), ("y", mty[string])]
      output := mty[regex] }

def reConcatFunc : LFunc BooleLParams :=
    { name := "Re.Concat",
      typeArgs := [],
      inputs := [("x", mty[regex]), ("y", mty[regex])]
      output := mty[regex] }

def reStarFunc : LFunc BooleLParams :=
    { name := "Re.Star",
      typeArgs := [],
      inputs := [("x", mty[regex])]
      output := mty[regex] }

def rePlusFunc : LFunc BooleLParams :=
    { name := "Re.Plus",
      typeArgs := [],
      inputs := [("x", mty[regex])]
      output := mty[regex] }

def reLoopFunc : LFunc BooleLParams :=
    { name := "Re.Loop",
      typeArgs := [],
      inputs := [("x", mty[regex]), ("n1", mty[int]), ("n2", mty[int])]
      output := mty[regex] }

def reUnionFunc : LFunc BooleLParams :=
    { name := "Re.Union",
      typeArgs := [],
      inputs := [("x", mty[regex]), ("y", mty[regex])]
      output := mty[regex] }

def reInterFunc : LFunc BooleLParams :=
    { name := "Re.Inter",
      typeArgs := [],
      inputs := [("x", mty[regex]), ("y", mty[regex])]
      output := mty[regex] }

def reCompFunc : LFunc BooleLParams :=
    { name := "Re.Comp",
      typeArgs := [],
      inputs := [("x", mty[regex])]
      output := mty[regex] }

def reNoneFunc : LFunc BooleLParams :=
    { name := "Re.None",
      typeArgs := [],
      inputs := []
      output := mty[regex] }
...
def polyOldFunc : LFunc BooleLParams :=
    { name := "old",
      typeArgs := ["a"],
      inputs := [((BooleIdent.locl "x"), mty[%a])]
      output := mty[%a]}

/- A `Map` selection function with type `∀k, v. Map k v → k → v`. -/
def mapSelectFunc : LFunc BooleLParams :=
   { name := "select",
     typeArgs := ["k", "v"],
...
def mapUpdateFunc : LFunc BooleLParams :=
   { name := "update",
     typeArgs := ["k", "v"],
...
instance : Coe String BooleLParams.Identifier where
  coe | s => ⟨s, .unres⟩

def emptyTriggersFunc : LFunc BooleLParams :=
    { name := "Triggers.empty",
      typeArgs := [],
      inputs := [],
      output := mty[Triggers],
...
def addTriggerGroupFunc : LFunc BooleLParams :=
...
def emptyTriggerGroupFunc : LFunc BooleLParams :=
...
def addTriggerFunc : LFunc BooleLParams :=
...
open Lean in
scoped macro "ExpandBVOpFuncNames'" "[" sizes:num,* "]" : term => do
  let mut allOps := #[]
  for size in sizes.getElems do
    let s := size.getNat.repr
    let ops := BVOpNames.map (mkIdent ∘ (.str (.str .anonymous "Boole")) ∘ (s!"bv{s}" ++ · ++ "Func"))
    allOps := allOps ++ ops.toArray
  `([$(allOps),*])

def bvConcatFunc (size : Nat) : LFunc BooleLParams :=
...
def bvExtractFunc (size hi lo: Nat) : LFunc BooleLParams :=
...
def Factory : @Factory BooleLParams := #[
  intAddFunc,
  intSubFunc,
  intMulFunc,
  intDivFunc,
  ...
  @intLtFunc BooleLParams _,
  @intLeFunc BooleLParams _,
  @intGtFunc BooleLParams _,
  @intGeFunc BooleLParams _,
  ...
  @boolAndFunc BooleLParams _,
  @boolOrFunc BooleLParams _,
  @boolImpliesFunc BooleLParams _,
  @boolEquivFunc BooleLParams _,
  @boolNotFunc BooleLParams _,
...
elab "DefBVOpFuncExprs" "[" sizes:num,* "]" : command => do
  for size in sizes.getElems do
    ...
    for op in BVOpNames do
      ...
      let funcName := mkIdent (.str (.str .anonymous "Boole") s!"bv{s}{op}Func")
      elabCommand (← `(def $opName : Expression.Expr := ($funcName).opExpr))

instance : Inhabited BooleLParams.Metadata where
  default := ()
...
instance : Inhabited (⟨ExpressionMetadata, BooleIdent⟩: LExprParams).Metadata where
  default := ()
...
def boolAndOp : Expression.Expr := @boolAndFunc.opExpr BooleLParams _
def boolOrOp : Expression.Expr := @boolOrFunc.opExpr BooleLParams _
def boolImpliesOp : Expression.Expr := @boolImpliesFunc.opExpr BooleLParams _
def boolEquivOp : Expression.Expr := @boolEquivFunc.opExpr BooleLParams _
def boolNotOp : Expression.Expr := @boolNotFunc.opExpr BooleLParams _
...
/--
Get all the built-in functions supported by Boole.
-/
def builtinFunctions : Array String :=
  Factory.map (fun f => BooleIdent.toPretty f.name)
